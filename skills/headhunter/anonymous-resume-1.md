...
任職期間：2023年11月 ~ 2024年11月

職稱：CTO 助理 / PM 專案經理

工作內容：
• 專案管理：依照產品規格書規劃開發順序、追蹤進度
• 開發協調：依照產品需求規格書溝通開發需求、API 串接
• 團隊管理：負責 RD/PM/QA 團隊管理，分配工作優先順序
• Apple / Google 上架管理

成就/專案：
• 管理 RD/PM/QA 團隊，確保專案如期交付
...
Previous Package  年薪約 100 萬（月薪 80-90K）

Current Package   年薪約 72 萬（轉職過渡期）

期待薪資          月薪 7.5 萬 up，年薪 120~150 萬

備註：候選人因轉職至遊戲產業累積相關經驗（撲克 APP 產品規劃），短期內接受較低薪資，現尋求回歸正常薪資水平。
...
【推薦理由】
此候選人目前在遊戲公司擔任 PM，負責撲克 APP 產品規劃，與貴司產品類型高度相關，可立即上手。

核心優勢：
• 產品經驗直接相關（熟悉撲克 APP 架構）
• Product Manager 定位明確（PRD 撰寫、競品分析）
• 跨部門協作能力強
• 產品從 0-1 經驗
• 技術理解能力佳
...
### 步驟 4：變數替換
```python
resume = template.replace('Sebastian', candidate_code)
resume = resume.replace('1992', candidate.birth_year)
resume = resume.replace('34', calculate_age(candidate.birth_year))
# ... 替換所有變數
```
