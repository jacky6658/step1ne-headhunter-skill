...
資訊安全、Penetration Test、Python、CTF、密碼學、零知識證明、區塊鏈
...
職稱：Turing Space Tech Researcher

工作內容：
• Turing Space Tech Researcher
...
On Board Date: One month notice period
...
### 步驟 4：變數替換
```python
resume = template.replace('Michael', candidate_code)
resume = resume.replace('1990', candidate.birth_year)
resume = resume.replace('36', calculate_age(candidate.birth_year))
# ... 替換所有變數
```
